...
class Leadwerk_Content_Schema {
	...
	public static function get_groups() {
		...
		$groups = array_merge( $groups, self::get_pascal_roth_groups() );

		return $groups;
	}

	/**
	 * Generic Pascal Roth static-page group.
	 *
	 * The Pascal Roth static export is section-rich but not built around the
	 * old ACM component vocabulary. Keep the editor stable by importing every
	 * source section as an editable raw HTML block with explicit media fields.
	 *
	 * @return array<string,array<string,mixed>>
	 */
	protected static function get_pascal_roth_groups() {
		return array(
			'pascal_page_sections' => array(
				'label'            => 'Pascal Roth Seiteninhalt',
				'description'      => 'Jede Sektion einzeln bearbeiten: Ueberzeile, Titel, Text, Buttons, Karten und FAQ. Das HTML-Template bleibt erhalten; Aenderungen werden beim Rendern eingepflegt.',
				'source_keys'      => self::get_pascal_roth_source_keys(),
				'dynamic_sections' => true,
				'layouts'          => array(
					'section' => self::layout_pascal_static_section(),
				),
			),
		);
	}
	...
	/**
	 * Pascal Roth source keys handled by the generic static renderer.
	 *
	 * @return string[]
	 */
	protected static function get_pascal_roth_source_keys() {
		return array(
			'pascal-home-v1',
			'pascal-ueber-v1',
			'pascal-kontakt-v1',
			'pascal-berufseinstieg-v1',
			'pascal-junge-familie-v1',
			'pascal-kinder-v1',
			'pascal-selbststaendigkeit-v1',
			'pascal-vermoegen-v1',
			'pascal-spaetstarter-v1',
			'pascal-spaetstarter-50plus-v1',
			'pascal-veraenderung-v1',
			'pascal-impressum-v1',
	...
	/**
	 * One editable Pascal Roth section.
	 *
	 * @return array<string,mixed>
	 */
	protected static function layout_pascal_static_section() {
		return array(
			'label'    => 'Sektion',
			'template' => 'pascal_static_section',
			'fields'   => array(
				'section_label'       => self::text( 'Sektionsname (nur Admin)' ),
				'section_id'          => array_merge(
					self::text( 'HTML-ID (Referenz)' ),
					array( 'advanced' => true )
				),
				'eyebrow'             => self::text( 'Ueberzeile / Overline' ),
				'heading'             => self::heading_html( 'Ueberschrift (H1/H2)' ),
				'subheading'          => self::textarea( 'Untertitel / Lead-Text' ),
				'intro'               => self::editor( 'Fliesstext / Einleitung' ),
				'buttons'             => self::repeater(
					'Buttons / Links',
					self::button_fields( 'cta' ),
					'Link hinzufuegen'
				),
				'cards'               => self::repeater(
					'Karten / Listeneintraege',
					array(
						'title'   => self::text( 'Titel' ),
						'text'    => self::editor( 'Text' ),
						'url'     => self::url( 'Link (optional)' ),
						'eyebrow' => self::text( 'Zusatzzeile (optional)' ),
					),
					'Karte hinzufuegen'
				),
				...
				'images'              => self::repeater(
					'Bilder in dieser Sektion',
					array(
						'source' => array_merge( self::text( 'Original src' ), array( 'advanced' => true ) ),
						'image'  => self::image( 'Bild' ),
						'alt'    => self::text( 'Alt-Text' ),
					),
					'Bild hinzufuegen'
				),
				'background_image'    => self::image( 'Hintergrundbild (optional)' ),
				'background_original' => array_merge(
					self::text( 'Original Hintergrundpfad' ),
					array( 'advanced' => true )
				),
				'html'                => array(
					'label'    => 'HTML-Template (Expertenmodus)',
					'type'     => 'html',
					'advanced' => true,
					'help'     => 'Nur bei Bedarf anpassen. Inhalte oben werden beim Rendern in dieses Template eingepflegt.',
				),
			),
		);
	}
