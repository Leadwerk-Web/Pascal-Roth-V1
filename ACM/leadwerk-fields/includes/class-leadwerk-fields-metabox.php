...
class Leadwerk_Fields_Metabox {
	...
	public static function render_sections_metabox( $post ) {
		...
		foreach ( $sections as $index => $section ) {
			...
			$display_label = trim( (string) ( $section['section_label'] ?? '' ) );
			if ( '' === $display_label ) {
				$display_label = $label;
			}
			...
			if ( ! empty( $section['section_id'] ) ) {
				echo ' <code>#' . esc_html( (string) $section['section_id'] ) . '</code>';
			}
			echo '</h3>';
	...
	private static function render_field( $name, $definition, $value, $id = '' ) {
		...
		$advanced  = ! empty( $definition['advanced'] );
		$field_class = 'leadwerk-field leadwerk-field-' . esc_attr( $type );
		if ( $advanced ) {
			$field_class .= ' leadwerk-field-advanced';
		}

		if ( $advanced ) {
			echo '<details class="' . esc_attr( $field_class ) . '">';
			echo '<summary>' . esc_html( $label ) . '</summary>';
			echo '<div class="leadwerk-field-advanced-inner">';
		} else {
			echo '<div class="' . esc_attr( $field_class ) . '">';
		}

		if ( 'checkbox' !== $type && ! $advanced ) {
		...
		switch ( $type ) {
			...
			case 'html':
				$rows = ! empty( $definition['advanced'] ) ? 10 : 18;
				echo '<textarea id="' . esc_attr( $id ) . '" name="' . esc_attr( $name ) . '" rows="' . esc_attr( (string) $rows ) . '" class="large-text code">' . esc_textarea( (string) $value ) . '</textarea>';
				if ( ! empty( $definition['help'] ) ) {
					echo '<p class="description">' . esc_html( (string) $definition['help'] ) . '</p>';
				} else {
		...
		if ( ! empty( $definition['help'] ) && 'html' !== $type ) {
			echo '<p class="description">' . esc_html( (string) $definition['help'] ) . '</p>';
		}
