...
class Leadwerk_Translation_API {
	...
	public static function get_record_by_public_slug( $language_code, $public_slug, $type_like = 'post_%' ) {
		...
		$public_slug = self::normalize_public_slug_path( $public_slug );
	...
	public static function upsert_element( $element_type, $element_id, $args ) {
		...
		$data     = array(
			...
			'public_slug'          => self::normalize_public_slug_path( $args['public_slug'] ?? ( $existing['public_slug'] ?? '' ) ),
	...
	public static function ensure_post_record( $post_id, $args = array() ) {
		...
		$is_home      = array_key_exists( 'is_home', $args )
			...
			: (
				...
				|| 'pascal-home-v1' === $source_key
				|| ( self::get_default_language() === $lang && (int) get_option( 'page_on_front' ) === (int) $post->ID )
			);
		$public_slug  = self::normalize_public_slug_path( $args['public_slug'] ?? ( $existing['public_slug'] ?? get_post_meta( $post->ID, self::META_PUBLIC_SLUG, true ) ) );
	...
	public static function link_posts( $source_post_id, $translated_post_id, $target_lang, $translated_is_root = false, $args = array() ) {
		...
		$source_record = self::ensure_post_record(
			...
			array(
				...
				'is_home'       => ! empty( $args['source_is_home'] ) || in_array( get_post_meta( $source_post->ID, 'leadwerk_source_key', true ), array( 'acm-home-v1', 'acm-index-v1', 'pascal-home-v1' ), true ) || (int) get_option( 'page_on_front' ) === (int) $source_post->ID,
				'status'        => 'complete',
			)
		);
	...
	public static function get_post_public_slug( $post_id ) {
		...
		if ( '' !== $public_slug ) {
			return self::normalize_public_slug_path( $public_slug );
		}
	...
	public static function update_post_identity( $post_id, $args = array() ) {
		...
		if ( array_key_exists( 'public_slug', $args ) && ! $is_home ) {
			$requested_slug = self::normalize_public_slug_path( $args['public_slug'] );
			if ( '' !== $requested_slug ) {
	 ...
	 *
	 * @param string $public_slug Raw public slug/path.
	 * @return string
	 */
	protected static function normalize_public_slug_path( $public_slug ) {
		$public_slug = trim( rawurldecode( str_replace( '\\', '/', (string) $public_slug ) ), '/' );
		if ( '' === $public_slug ) {
			return '';
		}

		$parts = preg_split( '#/+#', $public_slug );
	...
	protected static function generate_unique_public_slug( $public_slug, $lang, $post_id = 0 ) {
		$base = self::normalize_public_slug_path( $public_slug );
		if ( '' === $base ) {
			return '';
		}
	...
	protected static function public_slug_exists_for_other_post( $public_slug, $lang, $post_id = 0 ) {
		...
		$existing_id = (int) $wpdb->get_var(
			$wpdb->prepare(
				...
				sanitize_key( (string) $lang ),
				self::normalize_public_slug_path( $public_slug ),
				(string) $element_type,
				$post_id
			)
		);
	...
	protected static function derive_public_slug_from_post( $post, $lang ) {
		...
		if ( '' !== $stored ) {
			$slug = self::normalize_public_slug_path( $stored );
		} else {
			...
			$slug = self::normalize_public_slug_path( $slug );
		}
