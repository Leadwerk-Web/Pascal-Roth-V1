...
/**
 * Rewrite relative url(...) paths inside a static CSS file to absolute asset URLs.
 *
 * @param string $css          Raw CSS.
 * @param string $context_file CSS path relative to static root (e.g. lebensphasen/foo/page.css).
 * @return string
 */
function leadwerk_theme_rewrite_static_css_urls( $css, $context_file ) {
	$context_file = str_replace( '\\', '/', (string) $context_file );

	return (string) preg_replace_callback(
		'/url\\((["\']?)([^"\')]+)\\1\\)/i',
		static function ( $matches ) use ( $context_file ) {
			$raw = trim( html_entity_decode( (string) $matches[2], ENT_QUOTES | ENT_HTML5, 'UTF-8' ) );
			if ( '' === $raw || preg_match( '#^(?:https?:)?//#i', $raw ) || preg_match( '#^data:#i', $raw ) ) {
				return 'url(' . $matches[1] . $matches[2] . $matches[1] . ')';
			}

			$path = function_exists( 'leadwerk_theme_pascal_resolve_relative_path' )
				? leadwerk_theme_pascal_resolve_relative_path( $raw, $context_file )
				: ltrim( str_replace( '\\', '/', dirname( $context_file ) . '/' . $raw ), './' );

			return 'url("' . esc_url_raw( leadwerk_theme_static_asset_url( $path ) ) . '")';
		},
		(string) $css
	);
}
