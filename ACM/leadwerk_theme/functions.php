...
require_once LEADWERK_THEME_DIR . '/inc/social-preview.php';
$leadwerk_yoast_pascal_file = LEADWERK_THEME_DIR . '/inc/yoast-pascal-seo.php';
if ( is_file( $leadwerk_yoast_pascal_file ) ) {
	require_once $leadwerk_yoast_pascal_file;
}

$leadwerk_pascal_render_file = LEADWERK_THEME_DIR . '/inc/pascal-roth-render.php';
if ( is_file( $leadwerk_pascal_render_file ) ) {
	require_once $leadwerk_pascal_render_file;
}

$leadwerk_pascal_schema_file = LEADWERK_THEME_DIR . '/inc/pascal-structured-data.php';
if ( is_file( $leadwerk_pascal_schema_file ) ) {
	require_once $leadwerk_pascal_schema_file;
}
...
/**
 * Pascal Roth page context (header/footer chrome, no ACM overlays).
 *
 * @return bool
 */
function leadwerk_theme_is_pascal_page_context() {
	if ( ! function_exists( 'leadwerk_theme_is_pascal_source_key' ) || ! function_exists( 'leadwerk_theme_get_current_source_key' ) ) {
		return false;
	}

	return leadwerk_theme_is_pascal_source_key( leadwerk_theme_get_current_source_key() ) || is_404();
}

/**
 * Legacy short URLs from static HTML → imported WordPress slugs.
 *
 * @return void
 */
function leadwerk_theme_pascal_legacy_redirects() {
	...
	$map  = array(
		'berufseinstieg'              => 'lebensphasen/berufseinstieg',
		'junge-familie'               => 'lebensphasen/junge-familie',
		'kinder'                      => 'lebensphasen/kinder',
		'selbststaendigkeit'          => 'lebensphasen/selbststaendigkeit',
		'vermoegen-aufbauen'          => 'lebensphasen/vermoegen-aufbauen',
		'spaetstarter-50plus'         => 'lebensphasen/spaetstarter-50plus',
		'spaetstarter'                => 'lebensphasen/spaetstarter',
		'veraenderung'                => 'lebensphasen/veraenderung',
	);
...
add_action( 'template_redirect', 'leadwerk_theme_pascal_legacy_redirects', 0 );
...
/**
 * Resolve page.css for a Pascal shell (Datenschutz/Legal share impressum/page.css).
 *
 * @param string $source_key  Source key.
 * @param string $page_dir    Directory of the current shell file.
 * @param string $static_dir  Absolute static assets directory.
 * @return string Relative CSS path or empty.
 */
function leadwerk_theme_resolve_pascal_page_css_path( $source_key, $page_dir, $static_dir ) {
	$candidates = array();

	if ( '' !== $page_dir && '.' !== $page_dir ) {
		$candidates[] = $page_dir . '/page.css';
	}

	if ( in_array( (string) $source_key, array( 'pascal-datenschutz-v1', 'pascal-legal-v1' ), true ) ) {
		$candidates[] = 'impressum/page.css';
	}

	foreach ( $candidates as $relative_path ) {
		if ( is_file( trailingslashit( $static_dir ) . $relative_path ) ) {
...
/**
 * Whether the current request renders the Pascal kontakt page.
 *
 * @return bool
 */
function leadwerk_theme_is_pascal_kontakt_page() {
	if ( function_exists( 'leadwerk_theme_get_current_source_key' ) && 'pascal-kontakt-v1' === leadwerk_theme_get_current_source_key() ) {
		return true;
	}
	...
	return $queried instanceof WP_Post && 'kontakt' === $queried->post_name;
}

/**
 * Force WPForms "No styling" on Pascal kontakt (page.css provides layout).
 *
 ...
 * @return mixed
 */
function leadwerk_theme_wpforms_setting_on_pascal_kontakt( $value, $key, $default = null, $option = null ) {
	if ( 'disable-css' !== $key || ! leadwerk_theme_is_pascal_kontakt_page() ) {
		return $value;
	}

	return '3';
}
add_filter( 'wpforms_setting', 'leadwerk_theme_wpforms_setting_on_pascal_kontakt', 10, 4 );

/**
 * Dequeue any remaining WPForms styles after plugin enqueue.
 */
function leadwerk_theme_enqueue_pascal_kontakt_wpforms_overrides() {
	if ( ! leadwerk_theme_is_pascal_kontakt_page() ) {
		return;
	}

	$handles = array(
		'wpforms-modern-full',
		'wpforms-pro-modern-full',
		'wpforms-full',
		'wpforms-base-full',
		'wpforms-base',
		'wpforms-no-styles',
	...
	if ( wp_style_is( 'leadwerk-pascal-kontakt-wpforms-overrides', 'enqueued' ) ) {
		return;
	}

	wp_register_style(
		'leadwerk-pascal-kontakt-wpforms-overrides',
		false,
		array( 'leadwerk-pascal-page' ),
		defined( 'LEADWERK_THEME_VERSION' ) ? LEADWERK_THEME_VERSION : '1.0.0'
	);
	wp_enqueue_style( 'leadwerk-pascal-kontakt-wpforms-overrides' );

	$static_base = function_exists( 'leadwerk_theme_get_static_source_base' ) ? leadwerk_theme_get_static_source_base() : array();
	$css_file    = isset( $static_base['dir'] ) ? trailingslashit( $static_base['dir'] ) . 'kontakt/page.css' : '';

	if ( is_file( $css_file ) ) {
		$css_body = (string) file_get_contents( $css_file );
		$marker   = 'WPForms inside Pascal kontakt';
		$start    = strpos( $css_body, $marker );

		if ( false !== $start ) {
			$block_start = strrpos( substr( $css_body, 0, $start ), '/*' );
			$css_body    = false === $block_start ? substr( $css_body, $start ) : substr( $css_body, $block_start );
		}

		if ( '' !== trim( $css_body ) && function_exists( 'leadwerk_theme_rewrite_static_css_urls' ) ) {
			$css_body = leadwerk_theme_rewrite_static_css_urls( $css_body, 'kontakt/page.css' );
		}

		if ( '' !== trim( $css_body ) ) {
			wp_add_inline_style( 'leadwerk-pascal-kontakt-wpforms-overrides', $css_body );
		}
	}
}
add_action( 'wp_enqueue_scripts', 'leadwerk_theme_enqueue_pascal_kontakt_wpforms_overrides', 9999 );
add_action( 'wp_print_styles', 'leadwerk_theme_enqueue_pascal_kontakt_wpforms_overrides', 9999 );
...
function leadwerk_theme_enqueue_assets() {
	...
	$current_source_key = leadwerk_theme_get_current_source_key();

	if ( ( function_exists( 'leadwerk_theme_is_pascal_source_key' ) && leadwerk_theme_is_pascal_source_key( $current_source_key ) ) || is_404() ) {
		$source_file = function_exists( 'leadwerk_theme_pascal_current_source_file' ) ? leadwerk_theme_pascal_current_source_file() : '';
		$static_base = leadwerk_theme_get_static_source_base();
		$static_dir  = trailingslashit( $static_base['dir'] );
		$static_url  = trailingslashit( $static_base['url'] );

		wp_enqueue_style( 'leadwerk-theme-core', get_stylesheet_uri(), array(), LEADWERK_THEME_VERSION );
		wp_enqueue_style( 'leadwerk-pascal-fonts', 'https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap', array(), null );

		if ( is_file( $static_dir . 'styles.css' ) ) {
			wp_enqueue_style( 'leadwerk-pascal-styles', $static_url . 'styles.css', array( 'leadwerk-theme-core', 'leadwerk-pascal-fonts' ), (string) filemtime( $static_dir . 'styles.css' ) );
			wp_add_inline_style(
				'leadwerk-pascal-styles',
				'.pascal-bg-field{background-image:var(--leadwerk-bg-image)!important}.pascal-bg-field::before{background-image:var(--leadwerk-bg-image)!important}.admin-bar .nav-wrapper{top:32px}@media(max-width:782px){.admin-bar .nav-wrapper{top:46px}}'
			);
		}

		$page_dir = trim( dirname( (string) $source_file ), './' );
		$page_css = leadwerk_theme_resolve_pascal_page_css_path( $current_source_key, $page_dir, $static_dir );
		if ( '' !== $page_css ) {
			$css_file = $static_dir . $page_css;
			$css_body = is_file( $css_file ) ? (string) file_get_contents( $css_file ) : '';
			if ( '' !== $css_body && function_exists( 'leadwerk_theme_rewrite_static_css_urls' ) ) {
				$css_body = leadwerk_theme_rewrite_static_css_urls( $css_body, $page_css );
			}
			wp_register_style( 'leadwerk-pascal-page', false, array( 'leadwerk-pascal-styles' ), is_file( $css_file ) ? (string) filemtime( $css_file ) : LEADWERK_THEME_VERSION );
			wp_enqueue_style( 'leadwerk-pascal-page' );
			if ( '' !== $css_body ) {
				wp_add_inline_style( 'leadwerk-pascal-page', $css_body );
			}
		}

		if ( is_file( $static_dir . 'script.js' ) ) {
			wp_enqueue_script( 'leadwerk-pascal-script', $static_url . 'script.js', array(), (string) filemtime( $static_dir . 'script.js' ), true );
		}

		if ( '' !== $page_dir && '.' !== $page_dir ) {
			$page_js = $page_dir . '/page.js';
			if ( is_file( $static_dir . $page_js ) ) {
				wp_enqueue_script( 'leadwerk-pascal-page', $static_url . $page_js, array( 'leadwerk-pascal-script' ), (string) filemtime( $static_dir . $page_js ), true );
			}
		}
...
function leadwerk_theme_get_yoast_analysis_content( $post_id ) {
	...
	if ( function_exists( 'leadwerk_theme_maybe_sync_pascal_yoast_meta' ) ) {
		leadwerk_theme_maybe_sync_pascal_yoast_meta( $post_id );
	}

	if ( function_exists( 'leadwerk_theme_build_yoast_analysis_html' ) ) {
		return leadwerk_theme_build_yoast_analysis_html( $post_id );
	}
...
function leadwerk_theme_register_blocks() {
	$blocks = array(
		...
		'leadwerk-pascal-page' => 'leadwerk_theme_render_page_block',
...
if ( '' === $body_class && '' !== $source_key && function_exists( 'leadwerk_theme_is_pascal_source_key' ) && leadwerk_theme_is_pascal_source_key( $source_key ) && function_exists( 'leadwerk_theme_pascal_body_class' ) ) {
$body_class = (string) leadwerk_theme_pascal_body_class( $source_key );
}
...
if ( 'acm-index-v1' === $source_key || 'acm-home-v1' === $source_key || 'pascal-home-v1' === $source_key ) {
$classes[] = 'home';
}
if ( 'acm-404-v1' === $source_key || 'pascal-404-v1' === $source_key ) {
$classes[] = 'page-404';
$classes[] = 'header-scrolled';
}
...
function leadwerk_theme_favicon() {
	...
	if ( function_exists( 'leadwerk_theme_is_pascal_source_key' ) && ( leadwerk_theme_is_pascal_source_key( leadwerk_theme_get_current_source_key() ) || is_404() ) ) {
		echo '<link rel="icon" type="image/webp" href="' . esc_url( leadwerk_theme_static_asset_url( 'Fotos/cropped-Favicon.webp' ) ) . '">' . "\n";
		return;
	}
...
function leadwerk_theme_head_meta() {
	...
	if ( class_exists( 'Leadwerk_Translation_API' ) ) {
		...
		if ( ! empty( $translations['en'] ) ) {
			$en_post = get_post( (int) $translations['en'] );
			if ( $en_post instanceof WP_Post && 'publish' === $en_post->post_status ) {
				echo '<link rel="alternate" hreflang="en" href="' . esc_url( get_permalink( $en_post ) ) . '">' . "\n";
			}
		}
		if ( $x_default ) {
...
function leadwerk_theme_render_page_block() {
	...
	if ( ! empty( $group['dynamic_sections'] ) && function_exists( 'leadwerk_theme_render_pascal_page_group' ) ) {
		return leadwerk_theme_render_pascal_page_group( $group, $value, $post_id );
	}
...
function leadwerk_theme_render_header_block() {
	if ( function_exists( 'leadwerk_theme_is_pascal_source_key' )
		&& ( leadwerk_theme_is_pascal_source_key( leadwerk_theme_get_current_source_key() ) || is_404() )
		&& function_exists( 'leadwerk_theme_render_pascal_header_block' )
	) {
		return leadwerk_theme_render_pascal_header_block();
	}
...
function leadwerk_theme_render_footer_block() {
	if ( function_exists( 'leadwerk_theme_is_pascal_source_key' )
		&& ( leadwerk_theme_is_pascal_source_key( leadwerk_theme_get_current_source_key() ) || is_404() )
		&& function_exists( 'leadwerk_theme_render_pascal_footer_block' )
	) {
		return leadwerk_theme_render_pascal_footer_block();
	}
...
function leadwerk_theme_prepare_section_html( $html ) {
	if ( leadwerk_theme_section_has_pascal_contact_form( $html ) ) {
		$form_markup = leadwerk_theme_wrap_pascal_contact_form_markup( leadwerk_theme_get_contact_form_markup() );
		$replaced    = preg_replace( leadwerk_theme_pascal_contact_form_pattern(), $form_markup, (string) $html, 1, $count );
...
/**
 * Whether HTML contains the Pascal contact form shell.
 *
 * @param string $html HTML fragment.
 * @return bool
 */
function leadwerk_theme_section_has_pascal_contact_form( $html ) {
	return false !== strpos( (string) $html, 'kontakt-form' )
		|| false !== strpos( (string) $html, 'id="contact-form"' );
}

/**
 * Regex pattern for Pascal kontakt form replacement.
 *
 * @return string
 */
function leadwerk_theme_pascal_contact_form_pattern() {
	return '#<form\\b[^>]*(?:class="[^"]*\\bkontakt-form\\b[^"]*"|id="contact-form")[^>]*>.*?</form>#si';
}

/**
 * Wrap WPForms output in Pascal kontakt layout classes.
 *
 * @param string $markup Form markup from WPForms or fallback.
 * @return string
 */
function leadwerk_theme_wrap_pascal_contact_form_markup( $markup ) {
	$markup = trim( (string) $markup );
	if ( '' === $markup ) {
		return $markup;
	}

	if ( false === stripos( $markup, 'wpforms' ) ) {
		return $markup;
	}

	$phone_hint = '';
	if ( function_exists( 'leadwerk_theme_get_option_value' ) ) {
		$phone = trim( (string) leadwerk_theme_get_option_value( 'company_phone', '07249 3260' ) );
		if ( '' !== $phone ) {
			$tel_href   = preg_replace( '/[^\d+]/', '', $phone );
			$phone_hint = '<p class="kontakt-form__hint">Alternativ: <a href="tel:' . esc_attr( $tel_href ) . '" class="text-link">' . esc_html( $phone ) . '</a></p>';
		}
	}

	if ( '' !== $phone_hint && preg_match( '#<div class="wpforms-submit-container"[^>]*>.*?</div>#si', $markup ) ) {
		$markup = preg_replace(
			'#(<div class="wpforms-submit-container"[^>]*>.*?</div>)#si',
			'<div class="kontakt-form__actions kontakt-form__actions--wpforms">$1' . $phone_hint . '</div>',
			$markup,
			1
		);
	}

	return '<div class="kontakt-form kontakt-form--wpforms">' . $markup . '</div>';
}
...
function leadwerk_theme_is_legal_source_key( $source_key ) {
	return in_array( $source_key, array( 'acm-impressum-v1', 'acm-datenschutz-v1', 'pascal-impressum-v1', 'pascal-datenschutz-v1', 'pascal-legal-v1' ), true );
}
...
function leadwerk_theme_get_page_url( $source_key, $lang, $fallback = '#' ) {
	if ( class_exists( 'Leadwerk_Translation_API' ) ) {
		$url = Leadwerk_Translation_API::get_post_url_by_source_key( $source_key, $lang, '' );
		if ( '' !== $url ) {
			return $url;
		}
	} else {
	...
	if ( 'en' === $lang && function_exists( 'leadwerk_theme_is_pascal_source_key' ) && leadwerk_theme_is_pascal_source_key( $source_key ) ) {
		return leadwerk_theme_get_page_url( $source_key, 'de', $fallback );
	}
