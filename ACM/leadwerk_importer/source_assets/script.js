...
const NAV_LOGO_BLUE = 'Pascal Roth Logo_blau.webp';
const NAV_LOGO_DARK = 'Pascal Roth Logo.webp';
const NAV_LOGO_WHITE = 'Pascal_Roth_Logo_weiss.webp';

let navLogoBaseURL = '';
let navLogoVariants = null;

function navLogoUploadFilename(sourceFilename, baseURL) {
  if (/\/wp-content\/uploads\//i.test(baseURL)) {
    return sourceFilename.replace(/ /g, '-');
  }
...
function initNavLogoBaseFromMarkup() {
  ...
  const datasetBlue = img.getAttribute('data-logo-blue');
  const datasetDark = img.getAttribute('data-logo-dark');
  const datasetWhite = img.getAttribute('data-logo-white');
  if (datasetBlue && datasetDark && datasetWhite) {
    navLogoVariants = {
      blue: datasetBlue,
      dark: datasetDark,
      white: datasetWhite,
...
function setNavBarLogo(scrolled) {
  ...
  if (navLogoVariants) {
    next = isLegalPage
      ? (scrolled ? navLogoVariants.dark : navLogoVariants.white)
      : (scrolled ? navLogoVariants.dark : navLogoVariants.blue);
  } else if (navLogoBaseURL) {
    const file = isLegalPage
      ? (scrolled ? NAV_LOGO_DARK : NAV_LOGO_WHITE)
      : (scrolled ? NAV_LOGO_DARK : NAV_LOGO_BLUE);
    next = new URL(navLogoUploadFilename(file, navLogoBaseURL), navLogoBaseURL).href;
  }
