...
$src = Join-Path $PSScriptRoot '..\..\Fotos\WordCloud_Aufbau.svg'
...
$s = [regex]::Replace(
  $s,
  '<svg\s+id="Ebene_1"',
  '<svg id="vma-wordcloud-svg" class="vma-wordcloud__svg" focusable="false" aria-label="Themenwolke Vermoegensaufbau"',
  1
)

$matches = [regex]::Matches($s, '(?s)<text(\s[^>]*)>(.*?)</text>')
if ($matches.Count -eq 0) {
  throw 'Kein <text> in WordCloud_Aufbau.svg gefunden. Export pruefen (Pfade vs. Text).'
}
