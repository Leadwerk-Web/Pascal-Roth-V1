<?php
/**
 * Main importer for Leadwerk static site pages, media and translations.
 *
...
class Leadwerk_Importer {
	...
	protected function get_canonical_shell_html( $page_config ) {
		$file_name = ltrim( str_replace( '\\', '/', (string) ( $page_config['source_file'] ?? '' ) ), '/' );
		if ( '' === $file_name ) {
			return '';
		}
	...
	protected function record_preflight_source_asset_warnings() {
		...
		$required_assets = array(
			'styles.css',
			'script.js',
			'Fotos/Pascal Roth Logo_blau.webp',
			'Fotos/Pascal_Roth_Logo_weiss.webp',
			'Fotos/cropped-Favicon.webp',
		);
		foreach ( $required_assets as $rel ) {
			if ( ! $this->source_asset_exists( $root, $rel ) ) {
				Leadwerk_Logger::record_result(
					'warning',
					'Pascal Roth Quellasset fehlt: ' . $rel . '.',
					'preflight'
				);
			}
		}
	...
	protected function record_preflight_translation_seed_notice() {
		if ( $this->is_pascal_roth_manifest() ) {
			return;
		}
	...
	protected function apply_page_meta( $post_id, $payload, $page_config, $lang ) {
		...
		$source_key  = sanitize_key( (string) ( $page_config['source_key'] ?? '' ) );
		$seo_profile = function_exists( 'leadwerk_theme_get_pascal_seo_profile' )
			? leadwerk_theme_get_pascal_seo_profile( $source_key )
			: array();
		...
		$meta_description = trim( (string) ( $page_config['meta_description'] ?? $seo_profile['meta_description'] ?? $payload['meta_description'] ?? '' ) );
		if ( '' !== $meta_description ) {
			update_post_meta( $post_id, 'leadwerk_meta_description', sanitize_text_field( $meta_description ) );
			update_post_meta( $post_id, '_yoast_wpseo_metadesc', sanitize_text_field( $meta_description ) );
		}

		$seo_title_source = trim( (string) ( $page_config['seo_title'] ?? $seo_profile['seo_title'] ?? $payload['document_title'] ?? '' ) );
		if ( '' !== $seo_title_source ) {
			$seo_title = $this->truncate_seo_title_for_yoast( $seo_title_source );
		...
		if ( 'en' !== $lang ) {
			$focus = trim( (string) ( $page_config['focus_keyphrase'] ?? $seo_profile['focus_keyphrase'] ?? '' ) );
		}
		if ( '' === $focus && 'en' === $lang ) {
			$focus = trim( (string) ( $page_config['focus_keyphrase'] ?? $seo_profile['focus_keyphrase'] ?? '' ) );
		}
		...
		$og_image_raw = trim( (string) ( $payload['og_image'] ?? '' ) );
		if ( '' !== $og_image_raw ) {
			$og_image_url = function_exists( 'leadwerk_theme_resolve_shell_og_image_url' )
				? leadwerk_theme_resolve_shell_og_image_url( $og_image_raw, (string) $page_config['source_file'] )
				: esc_url_raw( $og_image_raw );

			if ( '' !== $og_image_url ) {
				update_post_meta( $post_id, 'leadwerk_og_image', $og_image_url );
				...
				delete_post_meta( $post_id, '_yoast_wpseo_opengraph-image-id' );
				delete_post_meta( $post_id, '_yoast_wpseo_twitter-image-id' );
			}
		}
	...
	protected function fill_options() {
		...
		$is_pascal = $this->is_pascal_roth_manifest();
		$option_map = array(
			'header_logo'     => $is_pascal ? 'Fotos/Pascal Roth Logo_blau.webp' : 'assets/images/Logo-final-weiss-rz_svg.svg',
			'footer_logo'     => $is_pascal ? 'Fotos/Pascal_Roth_Logo_weiss.webp' : 'assets/images/Logo-final-weiss-rz.png',
			'footer_wordmark' => $is_pascal ? 'Fotos/Pascal_Roth_Logo_weiss.webp' : 'assets/images/Schriftzug.svg',
		);

		foreach ( $option_map as $field_name => $source_path ) {
			$attachment_id = $this->resolve_attachment( $source_path );
			if ( $attachment_id && ( $is_pascal || ! $this->leadwerk_get_option( $field_name ) ) ) {
				$this->leadwerk_update_option( $field_name, $attachment_id );
			}
		}

		if ( $is_pascal || ! $this->leadwerk_get_option( 'company_address' ) ) {
			$this->leadwerk_update_option( 'company_address', $is_pascal ? "Pascal Roth\nFinanzberatung Karlsruhe" : "ACM AIR CHARTER Luftfahrtgesellschaft mbH\nMontreal Ave. D415\n77836 Rheinmünster" );
		}

		if ( $is_pascal || ! $this->leadwerk_get_option( 'company_phone' ) ) {
			$this->leadwerk_update_option( 'company_phone', $is_pascal ? '07249 3260' : '+49 7229 3022-0' );
		}

		if ( $is_pascal || ! $this->leadwerk_get_option( 'company_email' ) ) {
			$this->leadwerk_update_option( 'company_email', $is_pascal ? 'pascal.roth@allfinanz.ag' : 'info@acm.aero' );
		}

		if ( $is_pascal || ! $this->leadwerk_get_option( 'google_maps_url' ) ) {
			$this->leadwerk_update_option( 'google_maps_url', $is_pascal ? '#' : 'https://www.google.com/maps/dir/?api=1&destination=Montreal%20Ave.%20D415%2C%2077836%20Rheinm%C3%BCnster%2C%20Deutschland' );
		}
		...
		if ( $is_pascal || ! $this->leadwerk_get_option( 'theme_strings_de' ) ) {
			$strings = $is_pascal ? $this->get_pascal_theme_strings( 'de' ) : $this->get_default_theme_strings( 'de' );
			$this->leadwerk_update_option( 'theme_strings_de', wp_json_encode( $strings, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT ) );
		}

		if ( $is_pascal || ! $this->leadwerk_get_option( 'theme_strings_en' ) ) {
			$strings = $is_pascal ? $this->get_pascal_theme_strings( 'en' ) : $this->get_default_theme_strings( 'en' );
			$this->leadwerk_update_option( 'theme_strings_en', wp_json_encode( $strings, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT ) );
		}
	...
	/**
	 * Whether the current manifest is the Pascal Roth import.
	 *
	 * @return bool
	 */
	protected function is_pascal_roth_manifest() {
		foreach ( (array) ( $this->manifest['pages'] ?? array() ) as $page ) {
			if ( 0 === strpos( sanitize_key( (string) ( $page['source_key'] ?? '' ) ), 'pascal-' ) ) {
				return true;
			}
		}
	...
	/**
	 * Return Pascal Roth specific theme option strings.
	 *
	 * @param string $lang Language code.
	 * @return array<string,string>
	 */
	protected function get_pascal_theme_strings( $lang ) {
		if ( 'en' === $lang ) {
			return array(
				'services_menu_label'           => 'Life stages',
				...
				'footer_topics_heading'         => 'Life stages',
				'footer_contact_heading'        => 'Contact',
				'footer_menu_home'              => 'Home',
				'footer_phone_prefix'           => 'Phone:',
				'footer_desc_home'              => 'Financial advice in Karlsruhe, aligned with your current stage of life.',
				'footer_desc_general'           => 'Pascal Roth supports people in and around Karlsruhe with personal financial planning.',
				'footer_desc_legal'             => 'Pascal Roth Finanzberatung - legal notice and privacy information.',
				...
				'wpforms_message_placeholder'   => 'What would you like to talk about?',
		...
		return array(
			'services_menu_label'            => 'Lebensphasen',
			...
			'footer_topics_heading'          => 'Lebensphasen',
			'footer_contact_heading'         => 'Kontakt',
			'footer_menu_home'               => 'Startseite',
			'footer_phone_prefix'            => 'Tel.:',
			'footer_desc_home'               => 'Finanzberatung in Karlsruhe, passend zu deiner aktuellen Lebensphase.',
			'footer_desc_general'            => 'Pascal Roth begleitet Menschen in und um Karlsruhe mit persönlicher Finanzplanung.',
			'footer_desc_legal'              => 'Pascal Roth Finanzberatung - Impressum und Datenschutz.',
			...
			'wpforms_message_placeholder'    => 'Worum geht es? Was beschäftigt dich gerade?',
	...
	protected function set_site_icon() {
		$is_pascal = $this->is_pascal_roth_manifest();
		if ( get_option( 'site_icon' ) && ! $is_pascal ) {
			return;
		}

		$attachment_id = $this->resolve_attachment( $is_pascal ? 'Fotos/cropped-Favicon.webp' : 'favicon-192x192.png' );
		if ( $attachment_id ) {
	...
	/**
	 * Whether a source asset exists (supports png/jpg/webp alternates).
	 *
	 * @param string $root Root directory.
	 * @param string $relative_path Relative path.
	 * @return bool
	 */
	protected function source_asset_exists( $root, $relative_path ) {
		foreach ( $this->get_source_path_candidates( $relative_path ) as $candidate ) {
	...
	/**
	 * Alternate source paths for raster images after WebP conversion.
	 *
	 * @param string $source_path Source path.
	 * @return string[]
	 */
	protected function get_source_path_candidates( $source_path ) {
		$candidates = array( trim( $source_path, '/' ) );
		...
		if ( in_array( $ext, array( 'png', 'jpg', 'jpeg' ), true ) ) {
			...
			$candidates[] = preg_replace( '/\.webp$/i', '.jpg', $source_path );
		}
	...
	protected function resolve_attachment( $source_path ) {
		foreach ( $this->get_source_path_candidates( $source_path ) as $candidate ) {
			if ( $this->media_importer ) {
				$attachment_id = (int) $this->media_importer->get_attachment_id_by_source( $candidate );
				if ( $attachment_id ) {
	...
	protected function get_default_block_content() {
		if ( $this->is_pascal_roth_manifest() ) {
			return '<!-- wp:acf/leadwerk-pascal-page /-->';
		}
	...
	protected function get_public_slug( $page_config ) {
		...
		return $this->sanitize_public_slug_path( $page_config['slug'] ?? '' );
	}

	/**
	 * Sanitize a public URL path while preserving nested route separators.
	 *
	 * @param string $slug Raw public slug/path.
	 * @return string
	 */
	protected function sanitize_public_slug_path( $slug ) {
		$slug = trim( rawurldecode( str_replace( '\\', '/', (string) $slug ) ), '/' );
		if ( '' === $slug ) {
			return '';
		}
	...
	protected function get_translation_public_slug_for_link( $page_config, $post_id, $lang ) {
		...
		$fallback = $this->sanitize_public_slug_path( $this->get_public_slug( $page_config ) );
		$post_id  = (int) $post_id;
		...
		$stored_public_slug = $this->sanitize_public_slug_path( (string) get_post_meta( $post_id, $meta_key, true ) );
		...
		if ( class_exists( 'Leadwerk_Translation_API' ) && method_exists( 'Leadwerk_Translation_API', 'get_post_public_slug' ) ) {
			$api_public_slug = $this->sanitize_public_slug_path( (string) Leadwerk_Translation_API::get_post_public_slug( $post_id ) );
			if ( '' !== $api_public_slug && ( $api_public_slug === $fallback || false !== strpos( $api_public_slug, '/' ) || $api_public_slug !== $post_public_slug ) ) {
