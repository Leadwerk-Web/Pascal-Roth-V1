...
class Leadwerk_ACF_Filler {
	...
	public function build_page_payload_from_html( $page_config, $html, $lang = 'de' ) {
		...
		if ( ! empty( $group['dynamic_sections'] ) ) {
			return $this->build_dynamic_sections_payload( $payload, $page_config, $group, $xpath, $lang );
		}
	...
	/**
	 * Build a dynamic section payload for Pascal Roth static pages.
	 *
	 ...
	 * @return array<string,mixed>
	 */
	protected function build_dynamic_sections_payload( $payload, $page_config, $group, $xpath, $lang ) {
		unset( $lang );

		$field_name    = (string) ( $page_config['field_name'] ?? '' );
		$source_file   = (string) ( $page_config['source_file'] ?? '' );
		$layout_key    = 'section';
		$layout_schema = (array) ( $group['layouts'][ $layout_key ] ?? reset( $group['layouts'] ) );
		$section_nodes = $this->extract_body_sections( $xpath );
		$values        = array();
		$matches       = array();

		foreach ( $section_nodes as $index => $section_node ) {
			if ( ! $section_node instanceof DOMNode ) {
				continue;
			}

			$raw_value = $this->parse_pascal_static_section( $section_node, $source_file );
			if ( empty( $raw_value['background_image'] ) ) {
				$css_background = $this->find_pascal_css_background_for_section( $section_node, $source_file );
				if ( ! empty( $css_background['id'] ) ) {
					$raw_value['background_image']    = (int) $css_background['id'];
					$raw_value['background_original'] = (string) $css_background['source'];
				}
			}

			$values[] = $this->normalize_layout_value( $field_name, $layout_key, $raw_value );
			$matches[ 'section_' . ( $index + 1 ) ] = array(
				'matched_by'    => 'dynamic_section',
				'selector_used' => (string) ( $raw_value['section_id'] ?? '' ),
				'found'         => true,
				'source_index'  => $index,
			);
		}
		...
		$payload['value']              = $values;
		$payload['validation']         = $this->build_payload_validation( $field_name, $group, $payload['value'], count( $section_nodes ) );
		$payload['layout_diagnostics'] = $this->build_layout_diagnostics( $field_name, $group, $payload['value'], $matches );
	...
	protected function build_payload_validation( $field_name, $group, $value, $parsed_section_count = 0 ) {
		...
		if ( ! empty( $group['dynamic_sections'] ) ) {
			...
			$layout_schema = (array) reset( $group['layouts'] );

			$validation['expected_layout_count'] = count( $sections );
			$validation['parsed_layout_count']   = count( $sections );
			$validation['parsed_section_count']  = max( count( $sections ), (int) $parsed_section_count );
			$validation['missing_sections']      = 0;
			...
			foreach ( $sections as $section_index => $section ) {
				...
				foreach ( (array) ( $layout_schema['fields'] ?? array() ) as $field_key => $definition ) {
					$field_report = $this->summarize_definition_visibility(
						...
						$definition,
						'layout:section_' . ( $section_index + 1 ) . '.' . $field_key
					);
					$validation['visible_content_score'] += (int) $field_report['visible_count'];
					if ( ! empty( $field_report['empty_fields'] ) ) {
						$validation['empty_fields'] = array_merge( $validation['empty_fields'], $field_report['empty_fields'] );
				...
				if ( $section_has_visible_content ) {
					...
					$validation['empty_layouts'][] = 'section_' . ( $section_index + 1 );
				}
			}
	...
	protected function build_layout_diagnostics( $field_name, $group, $value, $matches = array() ) {
		...
		if ( ! empty( $group['dynamic_sections'] ) ) {
			...
			foreach ( $sections as $section_index => $section ) {
				$section       = is_array( $section ) ? $section : array();
				$schema        = Leadwerk_Content_Schema::get_layout( $field_name, (string) ( $section['acf_fc_layout'] ?? 'section' ) );
				$schema        = is_array( $schema ) ? $schema : (array) reset( $group['layouts'] );
				$visible_count = 0;
				$empty_fields  = array();
				...
				$diagnostics[] = array(
					'layout_key'                 => 'section',
					'label'                      => (string) ( $section['section_label'] ?? ( 'Sektion ' . ( $section_index + 1 ) ) ),
					'matched_by'                 => 'dynamic_section',
					'selector_used'              => (string) ( $section['section_id'] ?? '' ),
	...
	protected function parse_layout_value( $layout_key, $layout_schema, $section_node, $lang ) {
		...
		switch ( (string) ( $layout_schema['template'] ?? $layout_key ) ) {
			case 'pascal_static_section':
				return $this->parse_pascal_static_section( $section_node, '' );
			case 'hero_slider':
	...
	/**
	 * Parse one Pascal Roth static section as editable raw HTML plus media fields.
	 *
	 * @param DOMNode $section_node Section or main node.
	 * @param string  $source_file  Current HTML file relative to source root.
	 * @return array<string,mixed>
	 */
	protected function parse_pascal_static_section( $section_node, $source_file = '' ) {
		$section_id = $section_node instanceof DOMElement && $section_node->hasAttribute( 'id' )
			? (string) $section_node->getAttribute( 'id' )
			: '';
		$heading    = $this->text( $section_node, './/*[self::h1 or self::h2 or self::h3][1]' );
		$label      = '' !== $heading ? $heading : ( '' !== $section_id ? $section_id : $this->pascal_section_fallback_label( $section_node ) );
		$style      = $section_node instanceof DOMElement && $section_node->hasAttribute( 'style' )
			? (string) $section_node->getAttribute( 'style' )
			: '';
		$background = $this->resolve_context_image_from_style( $style, $source_file );
		$images     = array();

		foreach ( $this->query_nodes( $section_node, './/img' ) as $img_node ) {
			...
			$image = $this->resolve_context_image_field( $src, $source_file );
			$images[] = array(
				'source' => $src,
				'image'  => (int) $image['id'],
		...
		return array(
			...
			'eyebrow'             => $this->parse_pascal_section_eyebrow( $section_node ),
			'heading'             => $this->parse_pascal_section_heading( $section_node ),
			'subheading'          => $this->parse_pascal_section_subheading( $section_node ),
			'intro'               => $this->parse_pascal_section_intro( $section_node ),
			'buttons'             => $this->parse_pascal_section_buttons( $section_node ),
			'cards'               => $this->parse_pascal_section_cards( $section_node ),
			'faq_items'           => $this->parse_pascal_section_faq_items( $section_node ),
			'trust_badges'        => $this->parse_pascal_section_trust_badges( $section_node ),
			'html'                => $this->save_outer_html( $section_node ),
			'background_image'    => (int) $background['id'],
			'background_original' => (string) $background['source'],
			'images'              => $images,
		);
	}

	/**
	 * Parse Pascal section overline / eyebrow text.
	 *
	 * @param DOMNode $section_node Section node.
	 * @return string
	 */
	protected function parse_pascal_section_eyebrow( $section_node ) {
		return $this->first_available_text(
			$section_node,
			array(
				'.//*[contains(@class,"overline")][1]',
				'.//*[contains(@class,"eyebrow")][1]',
				'.//*[contains(@class,"legal-eyebrow")][1]',
			)
		);
	}

	/**
	 * Parse Pascal section primary heading inner HTML.
	 *
	 * @param DOMNode $section_node Section node.
	 * @return string
	 */
	protected function parse_pascal_section_heading( $section_node ) {
		return $this->first_available_html(
			$section_node,
			array(
				'.//h1[1]',
				'.//h2[1]',
	...
	/**
	 * Parse Pascal section lead / subtitle text.
	 *
	 * @param DOMNode $section_node Section node.
	 * @return string
	 */
	protected function parse_pascal_section_subheading( $section_node ) {
		$html = $this->first_available_html(
			$section_node,
			array(
				'.//*[contains(@class,"vma-hero__sub")][1]',
				'.//*[contains(@class,"__sub")][1]',
				'.//*[contains(@class,"legal-lead")][1]',
				'.//*[contains(@class,"faq-split__lead")][1]',
				'.//*[contains(@class,"__lead")][1]',
			)
		);

		return '' !== trim( wp_strip_all_tags( $html ) ) ? trim( wp_strip_all_tags( $html ) ) : '';
	}

	/**
	 * Parse Pascal section body / intro HTML.
	 *
	 * @param DOMNode $section_node Section node.
	 * @return string
	 */
	protected function parse_pascal_section_intro( $section_node ) {
		return $this->first_available_html(
			$section_node,
			array(
				'.//*[contains(@class,"bridge__text")][1]',
				'.//article[contains(@class,"legal-content")][1]',
				'.//*[contains(@class,"legal-section__body")][1]',
				'.//*[contains(@class,"kontakt-form-intro")][1]',
			)
		);
	}

	/**
	 * Parse Pascal section CTA buttons and text links.
	 *
	 * @param DOMNode $section_node Section node.
	 * @return array<int,array<string,string>>
	 */
	protected function parse_pascal_section_buttons( $section_node ) {
		$buttons = array();

		foreach ( $this->query_nodes( $section_node, './/a[contains(@class,"btn") or contains(@class,"text-link")]' ) as $node ) {
			if ( ! $node instanceof DOMElement ) {
				continue;
			}
			...
			$buttons[] = array(
				'cta_label'    => trim( preg_replace( '/\s+/', ' ', wp_strip_all_tags( $node->textContent ) ) ),
	...
	/**
	 * Parse Pascal section cards (bridge cards, testimonials, etc.).
	 *
	 * @param DOMNode $section_node Section node.
	 * @return array<int,array<string,string>>
	 */
	protected function parse_pascal_section_cards( $section_node ) {
		$cards = array();

		foreach ( $this->query_nodes( $section_node, './/*[contains(@class,"bridge__card")]' ) as $card ) {
			$cards[] = array(
				'title'   => $this->text( $card, './/strong[1]' ),
				'text'    => $this->html( $card, './/*[contains(@class,"bridge__card-copy")]//p[1]' ),
				'url'     => '',
				'eyebrow' => '',
			);
		}
		...
		foreach ( $this->query_nodes( $section_node, './/*[contains(@class,"testimonial-card")]' ) as $card ) {
			$cards[] = array(
				'title'   => $this->first_available_text(
					$card,
					array(
						'.//div/div/p[1]',
						'.//p[contains(@style,"font-weight:600")][1]',
					)
				),
				'text'    => $this->first_available_html(
					$card,
					array(
						'.//p[contains(@style,"italic")][1]',
						'.//p[contains(@class,"testimonial-text")][1]',
					)
				),
				'url'     => '',
				'eyebrow' => $this->text( $card, './/div/div/p[2]' ),
			);
		}
	...
	/**
	 * Parse Pascal FAQ accordion items.
	 *
	 * @param DOMNode $section_node Section node.
	 * @return array<int,array<string,string>>
	 */
	protected function parse_pascal_section_faq_items( $section_node ) {
		$items = array();

		foreach ( $this->query_nodes( $section_node, './/*[contains(@class,"faq-item")]' ) as $item ) {
			$items[] = array(
				'question' => $this->text( $item, './/*[contains(@class,"faq-trigger__label")][1]' ),
				'answer'   => $this->html( $item, './/*[contains(@class,"faq-answer")][1]' ),
			);
		}
	...
	/**
	 * Parse Pascal hero trust badges.
	 *
	 * @param DOMNode $section_node Section node.
	 * @return array<int,array<string,string>>
	 */
	protected function parse_pascal_section_trust_badges( $section_node ) {
		$badges = array();

		foreach ( $this->query_nodes( $section_node, './/*[contains(@class,"trust-badge")]' ) as $badge ) {
			$text = trim( preg_replace( '/\s+/', ' ', wp_strip_all_tags( $badge->textContent ) ) );
			if ( '' === $text ) {
				continue;
			}
			$badges[] = array( 'text' => $text );
		}
	...
	/**
	 * Return the first non-empty text match from a list of XPath queries.
	 *
	 * @param DOMNode  $context Context node.
	 * @param string[] $queries Queries.
	 * @return string
	 */
	protected function first_available_text( $context, $queries ) {
		foreach ( $queries as $query ) {
			$text = $this->text( $context, $query );
			if ( '' !== trim( $text ) ) {
				return $text;
			}
		}
	...
	/**
	 * Fallback label for a Pascal section.
	 *
	 * @param DOMNode $section_node Section node.
	 * @return string
	 */
	protected function pascal_section_fallback_label( $section_node ) {
		if ( $section_node instanceof DOMElement && $section_node->hasAttribute( 'class' ) ) {
			$classes = preg_split( '/\s+/', trim( (string) $section_node->getAttribute( 'class' ) ) );
	...
	/**
	 * Resolve a context-aware image field.
	 *
	 * @param string $src         Raw src/url.
	 * @param string $source_file Current HTML or CSS file.
	 * @return array{id:int,alt:string,source:string}
	 */
	protected function resolve_context_image_field( $src, $source_file = '' ) {
		$path = $this->resolve_context_asset_source_path( $src, $source_file );
		return array(
	...
	/**
	 * Resolve a context-aware background image from inline style.
	 *
	 * @param string $style       Inline style.
	 * @param string $source_file Current HTML or CSS file.
	 * @return array{id:int,alt:string,source:string}
	 */
	protected function resolve_context_image_from_style( $style, $source_file = '' ) {
		$src = '';
		if ( preg_match( '/url\\([\'"]?([^\'")]+)[\'"]?\\)/i', (string) $style, $matches ) ) {
			$src = (string) $matches[1];
		}

		return $this->resolve_context_image_field( $src, $source_file );
	}

	/**
	 * Find CSS background-image rules that target the current Pascal section.
	 *
	 * @param DOMNode $section_node Section node.
	 * @param string  $source_file  Current HTML file.
	 * @return array{id:int,source:string}
	 */
	protected function find_pascal_css_background_for_section( $section_node, $source_file ) {
		if ( ! $section_node instanceof DOMElement ) {
			return array( 'id' => 0, 'source' => '' );
		}

		$css_files = array( 'styles.css' );
		$dir       = trim( str_replace( '\\', '/', dirname( (string) $source_file ) ), './' );
		if ( '' !== $dir && '.' !== $dir ) {
			$css_files[] = $dir . '/page.css';
		}

		foreach ( array_unique( $css_files ) as $css_file ) {
			$full = $this->resolve_source_path( $css_file );
			if ( ! is_file( $full ) ) {
				continue;
			}
			...
			if ( ! preg_match_all( '/([^{}]+)\\{([^{}]*background-image\\s*:\\s*url\\([^)]+\\)[^{}]*)\\}/is', (string) $css, $matches, PREG_SET_ORDER ) ) {
				continue;
			}
			foreach ( $matches as $match ) {
				$selector = trim( (string) $match[1] );
				$body     = (string) $match[2];
				if ( ! $this->pascal_css_selector_targets_section( $selector, $section_node ) ) {
					continue;
				}
				if ( ! preg_match( '/background-image\\s*:\\s*url\\([\'"]?([^\'")]+)[\'"]?\\)/i', $body, $url_match ) ) {
					continue;
				}
				$image = $this->resolve_context_image_field( (string) $url_match[1], $css_file );
				if ( ! empty( $image['id'] ) ) {
					return array(
						'id'     => (int) $image['id'],
	...
	/**
	 * Test whether a simple CSS selector targets one section by id or class.
	 *
	 * @param string     $selector Selector list.
	 * @param DOMElement $section  Section element.
	 * @return bool
	 */
	protected function pascal_css_selector_targets_section( $selector, $section ) {
		$id      = $section->hasAttribute( 'id' ) ? (string) $section->getAttribute( 'id' ) : '';
		$classes = preg_split( '/\s+/', trim( (string) $section->getAttribute( 'class' ) ) );
		$classes = array_values( array_filter( (array) $classes ) );

		foreach ( preg_split( '/,/', (string) $selector ) as $part ) {
			$part = trim( preg_replace( '/::?[a-zA-Z0-9_-]+/', '', (string) $part ) );
			if ( '' !== $id && false !== strpos( $part, '#' . $id ) ) {
				return true;
			}
			foreach ( $classes as $class_name ) {
				if ( '' !== $class_name && false !== strpos( $part, '.' . $class_name ) ) {
					return true;
				}
			}
		}
	...
	/**
	 * Save one node as outer HTML.
	 *
	 * @param DOMNode $node Node.
	 * @return string
	 */
	protected function save_outer_html( $node ) {
	...
	protected function map_href_to_source_key( $href ) {
		...
		$pascal_map = array(
			'index.html'                                      => 'pascal-home-v1',
			'ueber-pascal/index.html'                        => 'pascal-ueber-v1',
			'kontakt/index.html'                             => 'pascal-kontakt-v1',
			'lebensphasen/berufseinstieg/index.html'         => 'pascal-berufseinstieg-v1',
			'lebensphasen/junge-familie/index.html'          => 'pascal-junge-familie-v1',
			'lebensphasen/kinder/index.html'                 => 'pascal-kinder-v1',
			'lebensphasen/selbststaendigkeit/index.html'     => 'pascal-selbststaendigkeit-v1',
			'lebensphasen/vermoegen-aufbauen/index.html'     => 'pascal-vermoegen-v1',
			'lebensphasen/spaetstarter/index.html'           => 'pascal-spaetstarter-v1',
			'lebensphasen/spaetstarter-50plus/index.html'    => 'pascal-spaetstarter-50plus-v1',
			'lebensphasen/veraenderung/index.html'           => 'pascal-veraenderung-v1',
			'impressum/index.html'                           => 'pascal-impressum-v1',
			'datenschutz/index.html'                         => 'pascal-datenschutz-v1',
			'danke/index.html'                               => 'pascal-danke-v1',
			'legal/index.html'                               => 'pascal-legal-v1',
			'404.html'                                       => 'pascal-404-v1',
		);
		$map = array_merge( $map, $pascal_map );
	...
	/**
	 * Resolve an asset path relative to the HTML/CSS file that referenced it.
	 *
	 * @param string $raw          Raw asset URL.
	 * @param string $context_file HTML/CSS file relative to source root.
	 * @return string
	 */
	protected function resolve_context_asset_source_path( $raw, $context_file = '' ) {
		$raw = trim( html_entity_decode( (string) $raw, ENT_QUOTES | ENT_HTML5, 'UTF-8' ) );
		if ( '' === $raw || preg_match( '#^(?:https?:)?//#i', $raw ) || preg_match( '#^(?:mailto:|tel:|data:|javascript:|#)#i', $raw ) ) {
			return '';
		}
		...
		$raw = preg_replace( '/[?#].*$/', '', $raw );
		$raw = str_replace( '\\', '/', rawurldecode( (string) $raw ) );
		$raw = preg_replace( '#^\./#', '', (string) $raw );
		if ( 0 === strpos( $raw, '/' ) ) {
			return $this->normalize_relative_path( $raw );
		}
		...
		return $this->normalize_relative_path( $combined );
	}

	/**
	 * Normalize a relative path with dot segments.
	 *
